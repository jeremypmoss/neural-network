    ...
    #%% XLSOptn
        if self.name.lower() == 'xlsoptn': # 31585
            ...
            df = pd.read_csv(path + '/vizier_IX_52_3xlsoptn_20220120.csv',
                             sep = ',', index_col = False,
                             header = 0,
                             usecols = ['zspec','uSDSSmag','gSDSSmag','rSDSSmag','iSDSSmag','zSDSSmag','WISE1mag','WISE2mag']
                             )
...
#%% Tidy the data
        ...
        if self.colours:
            ...
            colours = self.compute_colours(mgf); colours = colours.iloc[:, len(magnames):] # compute colours and add colours to magnitudes dataframe
            if redshift:
...
# create a DataLoader object for dataset 1
    data_loader = DataLoader(dropna = False,
                                ...
                                impute_method = 'max')
    dataset, datasetname, magnames, mags = data_loader.load_data('test')
    skymap, skymapname, skymapmagnames, skymapmags =data_loader.load_data('skymapper')
